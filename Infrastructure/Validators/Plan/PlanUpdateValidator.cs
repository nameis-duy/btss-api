...
{
    public class PlanUpdateValidator : AbstractValidator<PlanUpdate>
    {
        public PlanUpdateValidator(ITimeService timeService,
                                   ...
                                   SurchargeCreateValidator surchargeValidator)
        {
            RuleFor(p => p.PlanId).CustomAsync(async (planId, context, ct) =>
            {
                ...
                switch (plan.Status)
                {
                    case PlanStatus.CANCELED:
                        context.AddFailure(AppMessage.ERR_PLAN_UPDATE_STATUS_CANCEL);
                        return;
                    case PlanStatus.PENDING:
                        break;
                    default:
                        context.AddFailure(AppMessage.ERR_PLAN_UPDATE_STATUS_POST_PENDING);
                        return;
                }
            ...
            RuleFor(p => p.Schedule).Custom((schedule, context) =>
            {
                ...
                var maxIndex = (endAt - startAt).Days;
            ...
            RuleFor(p => p.SavedProviderIds).CustomAsync(async (providerIds, context, ct) =>
            {
                ...
                var destinationId = context.RootContextData[nameof(PlanCreate.DestinationId)];
